...
var RepoList = (function (_React$Component) {
	...
	_createClass(RepoList, [{
		...
		value: function componentWillMount() {
			...
			_superagent2["default"].get("https://api.github.com/search/repositories").query({ q: "angular" }).send().end(function (err, res) {
				console.log(res.body.items);
				_this2.setState({ data: res.body.items });
			});
		}
	}, {
		key: "render",
		value: function render() {
			var theList = this.state.data.map(function (repo) {
